...
2. The Tech Stack: Why These? Library Purpose: The "Why" Pandas Data Manipulation. Essential for handling DataFrames and CSVs. You can't do data science in Python without it.Matplotlib/SeabornVisualizationHumans are bad at reading rows of numbers; we need to see the trend. Scikit-Learn Machine Learning: The gold standard for robust, reproducible ML algorithms.
  
3. Step-by-Step Execution
1: Data Inspection. Before building a model, we perform a "Sanity Check."Action: Used df.info() and df.describe().
Why: To check for null values and understand the distribution (mean, min, max). If your data contains extreme outliers or missing values, your model will be skewed.
...
4: Model TrainingAction: Initialized LinearRegression() and called .fit(X_train, y_train).Why: This is where the math happens. The      algorithm performs Ordinary Least Squares (OLS), minimising the distance between the data points and the fitted line.
